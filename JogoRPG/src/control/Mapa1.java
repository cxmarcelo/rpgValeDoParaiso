...
public class Mapa1 extends JFrame  {
	...
	private BackgroundImagePanel mapa1; //ContentPane
	...
	public Mapa1(PersonagemPrincipal jogador, Bolsa bolsa) throws IOException {
	...
	private void InicializarComponentesMapa(PersonagemPrincipal jogador, Bolsa bolsa) throws IOException{
		...
		setResizable(false);
		BufferedImage myImage = ImageIO.read(Mapa1.class.getResource("../imgs/mapinha1.png"));
	...
	private void InicializarMonstros(int quantidade) {
		...
		for(int i = 0; i < quantidade; i++) {
			...
			if(i < quantidade/2) {
				Monstro m = new Monstro("Esqueleto "  + (1 + i), 3, "Fisico");
				monstros.add(m);
				...
				Monstro m = new Monstro("Esqueleto "  + (1 + i), 1, "Magico");
				monstros.add(m);
				JLabel lblMonstro = m.getMonstroUI();
				mapa1.add(lblMonstro);
